...
def plotDiNtw(Nodes,Arcs,Weights,PositionsN,fname):
    fig = plt.figure()
    DG = nx.DiGraph()
    for n in Nodes:
        DG.add_node(n)
#        DG.add_node(n,pos=PositionsN[n])
#    pos = nx.get_node_attributes(DG,'pos')
    pos = nx.circular_layout(DG)
#    nx.draw_networkx_nodes(DG,pos,node_color='lightgray',node_size=750,with_labels=True)
#    labels = {}
...
#    nx.draw_networkx_labels(DG,pos,labels,font_size=16)
    ...
    for weight in unique_weights:
        weighted_edges = [(node1,node2) for (node1,node2,edge_attr) in DG.edges(data=True) if edge_attr['weight']==weight]
#        width = weight*len(Nodes)*3.0/sum(all_weights)
        width = 5*weight
        nx.draw_networkx_edges(DG,pos,edgelist=weighted_edges,width=width)
    nx.draw(DG,pos,node_size=850,with_labels=True, node_color='lightgray',font_size=16)
#    pos=nx.get_node_attributes(G,'pos')
#    nx.draw(DG,pos)
...
E[2][1,2] = 0.75
E[2][2,0] = 0.5
E[0] = sum(E[k]*alpha[k] for k in range(1,KAmb))/(1.0-alpha[0])

for k in range(0,KAmb):
    fname = ('AmbE%i.'%k)+figext
    plotDiNtw(Nodes,Arcs,E[k],PositionsN,fname)
...
sol3d(X,Y,ExpCP,'AmbE-ExCP.',ptitle1,figext)
sol3d(X,Y,VarsCP,'AmbE-VarsCP.',ptitle2,figext)
sol3d(X,Y,VarsACP,'AmbE-VarsACP.',ptitle3,figext)
...
ofUniPolplt(xx,Ex_UniPoli,'AmbE-ExCPUP.'+figext,ptitle1)
ofUniPolplt(xx,Var_UniPoli,'AmbE-VarCPUP.'+figext,ptitle2)
ofUniPolplt(xx,VarA_UniPoli,'AmbE-VarACPUP.'+figext,ptitle3)
...
for nu in range(0,NTT):
    for kappa in range(0,NGG):
        ...
        fname='AmbE-FCP_%1.2f_%1.2f.'%(theta[nu],gamma[kappa])
        fnameUP='AmbE-FCPUP_%1.2f_%1.2f.'%(theta[nu],gamma[kappa])
